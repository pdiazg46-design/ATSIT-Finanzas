...
export default async function BalanceReportPage() {
    ...
    return (
        <div className="space-y-8">
            ...
            <header>
                <h2 className="text-3xl font-bold text-white">Balance de Proyectos</h2>
                <p className="text-slate-400">Comparativa financiera de proyectos activos (Desglose Neto + IVA)</p>
            </header>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="glass-card p-6 flex flex-col justify-between">
                    <div>
                        <p className="text-xs font-bold text-sky-400 uppercase tracking-widest mb-1">Total Saldo Neto</p>
                        <p className="text-2xl font-black text-white">{formatCurrency(totals.balance)}</p>
                    </div>
                </div>
                {/* Removed VAT Card */}
                <div className="glass-card p-6 flex flex-col justify-between border-emerald-500/20 bg-emerald-500/5 relative overflow-hidden">
                    {/* Icon can be kept or removed if redundant with export buttons, keeping for UI consistency */}
                    <div>
                        <p className="text-xs font-bold text-emerald-400 uppercase tracking-widest mb-1">Total en Banco</p>
                        <p className="text-3xl font-black text-white">{formatCurrency(totalBank)}</p>
                    </div>
                    <p className="text-xs text-emerald-500/60 mt-2">Saldo Real Disponible</p>
                </div>
            </section>
            ...
            <section className="glass-card overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-white/5 bg-white/5">
                                <th className="px-6 py-4">Proyecto</th>
                                <th className="px-6 py-4 text-right">Esperado</th>
                                <th className="px-6 py-4 text-right">Real (Neto)</th>
                                <th className="px-6 py-4 text-right">Gastos (Neto)</th>
                                {/* Removed VAT Header */}
                            ...
                            {reportData.map((row, i) => {
                                ...
                                return (
                                    <tr key={i} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-sm text-right text-slate-400">{formatCurrency(row.expectedIncome || 0)}</td>
                                        <td className="px-6 py-4 text-sm text-right text-emerald-400 font-bold">{formatCurrency(row.realIncomeNet)}</td>
                                        <td className="px-6 py-4 text-sm text-right text-rose-400 font-bold">{formatCurrency(row.expensesNet)}</td>
                                        {/* Removed VAT Cell */}
                                        <td className={`px-6 py-4 text-sm text-right font-bold ${row.balance >= 0 ? 'text-sky-400' : 'text-rose-400'}`}>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                <span className="text-xs font-bold text-slate-500">{perc.toFixed(0)}%</span>
                                            </div>
                                        </td>
                                    </tr>
                                );
                        ...
                        <tfoot>
                            <tr className="bg-white/5 border-t border-white/10 font-bold">
                                <td className="px-6 py-4 text-sm text-white uppercase tracking-wider">Totales</td>
                                <td className="px-6 py-4 text-sm text-right text-slate-300">{formatCurrency(totals.expectedIncome)}</td>
                                <td className="px-6 py-4 text-sm text-right text-emerald-400">{formatCurrency(totals.realIncomeNet)}</td>
                                <td className="px-6 py-4 text-sm text-right text-rose-400">{formatCurrency(totals.expensesNet)}</td>
                                {/* Removed VAT Footer */}
                                <td className={`px-6 py-4 text-sm text-right ${totals.balance >= 0 ? 'text-sky-400' : 'text-rose-400'}`}>
